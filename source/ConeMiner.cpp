...
//#   pragma omp parallel for
	...
	while(! stack.empty()){
      ...
      cone_value[b] += max_profit;
	    ...
	    for(int r=0;r<r_max && !skip_list[b];++r){ // iterate over resources
        ...
        // if cone violates entire allowable resource limits, dont take it
        if (res[b][r] > prob.getLimit(r,period) * double(1.0/sh.NUM_SUB_PERIODS) * sh.RES_CUTOFF){
          // add all successors to skip list
          std::vector<int> sub_stack;
          sub_stack.push_back(b);
          skip_list[b] = 1;
          while (!sub_stack.empty()){
            int block_idx = sub_stack.back();
            sub_stack.pop_back();
            // get graph node of current block
            const Node* curr = prob.graph.getNode(block_idx);
            // get successors to current block
            std::vector<Node*> connected = std::vector<Node*>();
            const int num_connected = curr->getConnected(FORWARD, connected);
            // iterate over successors
            for (size_t node = 0; node < num_connected; ++node){
              int node_idx = connected[node]->getID();
              // if successor not already skipped, add to stack
              if (!skip_list[node_idx]){
                skip_list[node_idx] = 1;
                sub_stack.push_back(node_idx);
              }
            }
          }
          break;
