...
int SinglePSolver::GRASPSolve(){
  ...
  for (int full_run = 0; full_run < sh.NUM_TOTAL_RUNS; ++full_run){
    ...
    std::cout << "selecting best seed.. " << std::flush;
    for (int i = 1; i < seeds.size(); ++i){
      if(seeds[i].obj > best_sol.obj){
        best_sol = seeds[i];
      }
    }
    std::cout << "done!" << std::endl;
    LocalSearch ls = LocalSearch(sh,probModel,include);
    GRASPSolver gs = GRASPSolver(sh,probModel,include);
    double old_obj = best_sol.obj;

    for (int pass = 0; pass < sh.NUM_PASSES; ++pass){

      qol::CpuTimer pass_timer;

      for (int t_0 = t_max-sh.WINDOW_SIZE; t_0 > 1; --t_0){
      std::cout << "\n*** RUNNING BACKWARD PASS "<< pass+1 << " of " << sh.NUM_PASSES << " (" << t_0 << "-" << t_0+sh.WINDOW_SIZE-1<<") ***" << std::endl;
      //   double sol_obj=0.0;
      //   for (int b = 0; b < nB; ++b){
      //     if (best_sol.x[b] < t_max){
      //       sol_obj += (*blocks)[b].getProfit(0) / pow(1+rate,best_sol.x[b]);
      //     }
      //   }

      //   std::cout << "test obj: " << sol_obj << std::endl;

        Sol_Int old_sol = best_sol;
        gs.solveWindow(best_sol,t_0);
        computeResUse(best_sol);

        std::cout << "\nObjective before window solve: " << old_sol.obj << std::endl;
        std::cout << "Objective after window solve: " << best_sol.obj << std::endl;
        try{
          std::cout << "\nChecking solution from solver...\n" << std::endl;
        ...
        seed_out[0] << best_sol.obj << std::endl;
        seed_out[1] << best_sol.obj << std::endl;
        time_out[0] << over_timer.elapsedSeconds() << std::endl;
        time_out[1] << over_timer.elapsedWallTime() << std::endl;

      }

      for (int t_0 = 0; t_0 < t_max - sh.WINDOW_SIZE; ++t_0){
        std::cout << "\n*** RUNNING FORWARD PASS "<< pass+1 << " of " << sh.NUM_PASSES << " (" << t_0 << "-" << t_0+sh.WINDOW_SIZE-1<<") ***" << std::endl;
        // double pre_solve_obj = best_sol.obj;
        Sol_Int old_sol = best_sol;
        gs.solveWindow(best_sol,t_0);
        computeResUse(best_sol);
        std::cout << "\nObjective before window solve: " << old_sol.obj << std::endl;
    ...
    std::cout << "### FINISHED GRASP HEURISTIC SEARCH ###" << std::endl;
    std::cout << "\nOld objective: " << old_obj << std::endl;
...
//     }
    ...
    // _log << iter << "," << sh.NUM_ITER << "," << sh.NUM_MOVES << ",";
    _log << sh.NUM_ITER << ",";
    _log << sh.NUM_SPLITS << "," << sh.SPLIT_FACTOR << "," << sh.SA_ITER << "," << sh.HEURISTIC_SEARCH << "," << sh.WINDOW_SEARCH_TIME << "," << sh.MERGE_POP_SIZE << "," << sh.MAX_SUB_SWAPS << "," << sh.SWAP_POP << std::endl;
    _log.close();
