...
Block::Block(){
	...
	_xyz.y=0;
	_xyz.z=0;
	_profit = std::vector<double>(1);
}
Block::~Block(){}
//enquiry
int Block::getID() const { return _id;}
coordinate Block::getXYZ() const { return _xyz;}

int Block::getX()const { return _xyz.x;}
int Block::getY()const { return _xyz.y;}
int Block::getZ()const { return _xyz.z;}

std::vector<int> * Block::getPreds() { return &_pred;}
int Block::getNumPred() const { return _pred.size();}
bool Block::isPred(int b) const {
	int m = _pred.size();
	bool YesNo = 0;
	for(int i =0; i<m; i++){
		if(_pred[i] == b){
...
double Block::getAttr(int i) const {return _attributes[i];}
int Block::getnAttr() const {return _attributes.size();}

double Block::getProfit(int d) const {return _profit[d];}
std::vector<double> * Block::getProfit(){return &_profit;}

double Block::getRCoef(int d, int r) const {return _rCoef[d][r];}
...
Daten::Daten(const char * filename, char prob){
	// Initialize
	...
	readData(filename);
	_nArcs = 0;
	for(int b=0; b<_nBlock; b++)
		_nArcs += _blocks[b].getNumPred(); // block precedence arcs

}
Daten::~Daten(){}
//enquiry
std::vector< std::vector< std::vector<int> > > * Daten::getLimit(){return &_rLimits;}
int Daten::getLimit(int r, int t, int i) const {return _rLimits[r][t][i];}

std::vector< std::vector<char> > * Daten::getResConstrType(){return &_rConstrType;}
char Daten::getResConstrType(int r, int t) const {return _rConstrType[r][t];}

std::vector<Block> * Daten::getBlock(){return &_blocks;}
//Block Daten::getBlock(int i) const {return _blocks[i];}
...
// open filename/*.extension(.xz/.zip/.gz/.bzip2)
// class FileReader {
// public:
// 	FILE *f;
// 	bool isPipe;
// 	std::string filename;
// 	FileReader(std::string dirname,std::string extension) : f(0),isPipe(false) {
// 		namespace fs == boost::filesystem;
// 		fs::path dir(dirname);
// 		if( ! fs::exists(dir) || !fs::is_directory(dir)) return;
// 		for( fs::directory_iterator file; file!= ...;){}
// 	}
// }
...
void Daten::readProb(const char * filename){
	...
	if (inputfile.is_open()){
		...
		while(!inputfile.eof()){
			...
			if(line.find("NAME:",foundPos) != std::string::npos) {
				foundPos = line.find(":",foundPos);
				foundPos=line.find_first_not_of(" \t",foundPos+1);
				foundEnd = line.find_first_of(" \t", foundPos+1);
				_name = line.substr(foundPos, foundEnd-foundPos);
			}else if(line.find("NBLOCKS", foundPos) != std::string::npos){
				foundPos = line.find(":",foundPos);
				foundPos = line.find_first_not_of(" \t,", foundPos+1);
				foundEnd = line.find_first_of(" \t", foundPos+1);
				...
				for(int i=0; i< _r_max*_t_max; i++){
					...
					strng = line.substr(foundPos, foundEnd-foundPos);
					bool interval = 0;
					if(strng.compare("G")==0) _rConstrType[r][p]='G';
